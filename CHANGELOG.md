...
- Two-step verification now applies to Plex sign-in too, not only to the password login.
...
- Dependencies updated: uvicorn 0.53.0, alembic 1.20.0, python-telegram-bot 21.11.1.
...
- Invites on a paid plan now record the first payment in reports instead of leaving a pending renewal to collect again.
...
- Reports: the renewal rate counts paid renewals (it read ~0%), and "already collected" is a share of the month total (it could exceed 100%).
- Users list: sorting now applies across pages, not only within the current one.
...
- The invite role dropdown now starts at `user` instead of `admin`, so nobody is invited as an administrator by accident.
...
[1.2.4]: https://github.com/Pantanet96/AccessFlow/releases/tag/v1.2.4
